...
class Admin extends CI_Controller {
    ...
    function view_logs() {
        if ($this->session->userdata('admin')) {
            ...
            $branchId = $this->db->get_where('tbl_admin',array('admin_username'=>$admin))->row()->branch_id;
            
            $config['base_url'] = base_url().'admin/view_logs';
            $config['total_rows'] = $this->db->get_where('tbl_log_manager',array('branch_id'=>$branchId))->num_rows();
            $config['per_page'] = 10;
            $config['num_links'] = 4;
            ...
            $data['logs'] = $this->db->where('branch_id',$branchId);
    ...
    function liveChat(){
         $data['page'] = 'admin/pages/livechat';
         $this->load->view('admin/admin_dash', $data);

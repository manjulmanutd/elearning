...
class Trainers extends CI_Controller {
    ...
    function message_trainer($trainerId) {
        $data['trainerDet'] = $this->trainer_model->getTrainerDetById($trainerId);
        $data['page'] = 'admin/pages/trainer/message_trainer';
        $this->load->view('admin/admin_dash', $data);
    }

    function send_message($trainerId) {

        $this->trainer_model->sendMessage($trainerId);
        redirect('trainers/list_trainer');
    }

     function messageTrainers() {
        $data['page'] = 'admin/pages/trainer/message_trainers';
        $this->load->view('admin/admin_dash', $data);
    }

      function send_message_trainers() {

        $this->trainer_model->sendMessageTrainers();
        redirect('trainers/viewTrainerMessages');
    }

    function viewTrainerMessages(){

        $data['messages'] = $this->trainer_model->getTrainerMessages();
        $data['page'] = 'admin/pages/trainer/view_message_trainers';
        $this->load->view('admin/admin_dash', $data);
    }

    function removeTrainersMessage(){
        $this->trainer_model->removeTrainersMessage();
        redirect('trainers/viewTrainerMessages');
    }

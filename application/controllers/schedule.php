...
class Schedule extends CI_Controller {
   ...
   {table_close}</table>{/table_close}'
    ...
    function edit_schedule_year(){
        $data['allCourses'] = $this->schedule_model->getAllCourses();
        $data['allTrainings'] = $this->schedule_model->getAllTrainingDetails();
        $data['timeSlots'] = $this->schedule_model->getAvalaibleTimeSlots();


        $data['page'] = 'admin/pages/schedulesByMonth/edit_schedule_year';
        $this->load->view('admin/admin_dash', $data);
    }
    ...
    function inspect() {
        ...
        $data['day'] = $day;
        //$result = $this->schedule_model->inspectWorkingDay($day, $date);
        /*if (empty($result)) {
    ...
    function configure_holidays() {
        $data['allHolidays'] = $this->schedule_model->getAllHolidays();
        $data['page'] = 'admin/pages/holidays/configure_holidays';
        $this->load->view('admin/admin_dash', $data);
    }

    function add_holiday() {
        $data['page'] = 'admin/pages/holidays/add_holiday';
        $this->load->view('admin/admin_dash', $data);
    }

    function insert_holiday() {

        $this->schedule_model->addHoliday();
        redirect('schedule/configure_holidays');
    }

    function remove_holiday() {
        $this->schedule_model->removeHoliday();
        redirect('schedule/configure_holidays');
    }
    
     function view_all_schedules(){
        $data['allCourses'] = $this->schedule_model->getAllCourses();
        $data['page'] = 'admin/pages/schedulesByMonth/list_courses_months';
        $this->load->view('admin/admin_dash',$data);
    }

    function listSchedulesByYear(){

        $data['allSchedules'] = $this->schedule_model->getSchedulesByYear();
        $data['page'] = 'admin/pages/schedulesByMonth/list_schedules_by_year';
        $this->load->view('admin/admin_dash',$data);
     ...
     function view_trainees($scheduleId){
        ...
        $data['page'] = 'admin/pages/schedulesByMonth/view_trainees';
      
        $this->load->view('admin/admin_dash',$data);
        
    }
    
    function view_feedback_course($userId,$scheduleId){
        $data['allFeedback'] = $this->schedule_model->getFeedbackByCourse($userId,$scheduleId);
        $data['page'] = 'admin/pages/schedulesByMonth/view_trainees';

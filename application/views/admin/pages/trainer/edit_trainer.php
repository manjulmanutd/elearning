...
<form action="<?php echo base_url();?>trainers/update_trainer" method="post" enctype="multipart/form-data" id="trainer_form">
...
<input type='text' name='contact'  <?php if($data['phone']){?>value="<?php echo  $data['phone']?>"<?php }?>/>
      ...
      <input type='text' name='email' class="required"  <?php if($data['email']){?>value="<?php echo  $data['email'];?>"<?php }?>/>
       ...
       <h3>isActive</h3>
<input type='checkbox' name='active' <?php if($data['status']==1){?> checked="checked" <?php }?>/>
<div class="seperator"></div>

    ...
    <label>Select Schedule to proceed:</label><select name="schedule_id" id="selectSchedule" onchange="selectSchedule()">
       <option value="0">Select a schedule to proceed</option>
        ...
        <?php foreach ($allSchedules as $schedule): ?>
            <option value="<?php echo $schedule->training_id; ?>"><?php echo $this->documentation_model->getCourseNameById($schedule->course_id)->course_name." ".$this->documentation_model->getLessonNameById($schedule->lesson_id)->lesson_name." ( ".date('d M Y',strtotime($schedule->training_date)).")"; ?></option>
        <?php endforeach; ?>
    </select></div>
